static PASSES:&str = "oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
fn part_1() {
	...
	let passwords = PASSES.split("\n");

	for pass in passwords {
		//println!("Checking password {}", pass);
		...
		'outer: for s in spl {
			words.push(s);
			let count = words.iter().filter(|&n| *n == s).count();
			if count > 1 {
				valid_flag = false;
				break 'outer;
			} 
	...
	println!("Part 1: {} valid passwords, {} invalid.", valid, invalid);
}
...
fn part_2() {
	...
	for pass in passwords {
		...
		'outer: for s in spl {
			// Create a hash for the current word
			let mut schars: Vec<char> = s.chars().collect();
			schars.sort();
			let shash: String = schars.into_iter().collect();

			for word in words.clone() {
				//Create a hash for the dictionary words, too
				let mut wchars: Vec<char> = word.chars().collect();
				wchars.sort();
				let whash: String = wchars.into_iter().collect();

				//Since we sorted all the letters to create a hash, if the hashes map this password is invalid
				if whash == shash {
					//println!("{} matches {} for {} and {}", shash, whash, s, word);
					valid_flag = false;
					break 'outer;
				}
			} 
	...
	println!("Part 2: {} valid passwords, {} invalid.", valid, invalid);
}
