    static ROWS:[[i32; 16]; 16] = [[157, 564, 120, 495, 194, 520, 510, 618, 244, 443, 471, 473, 612, 149, 506, 138],
...
fn part_1() {
    ...
    for i in 0..16 {
    	for j in 0..16 {
    		if ceiling < ROWS[i][j] {
    			ceiling = ROWS[i][j];
    		} else if floor > ROWS[i][j] {
    			floor = ROWS[i][j];
    		}
    	}
    	checksum += ceiling - floor;
    	ceiling = 0;
    	floor = 99999999;
    }
...
fn part_2() {
	...
	for i in 0..16 {
    	for j in 0..16 {
    		...
    		for k in 0..16 {
    			if j != k && !brk && (ROWS[i][j] % ROWS [i][k] == 0) {
    				let div = ROWS[i][j] / ROWS[i][k];
    				checksum += div;
    				brk = true;
    			}
    		}		
